...
/**
 * Everything on the homepage below the hero, exactly as it stands on /home3.
 *
 * It lives in its own component so the five hero studies (/home5–/home9) can
 * share one body. That is not a convenience: holding the entire page fixed
 * except the hero is what makes the comparison honest. If this file is edited,
 * all five studies move together, which is the intended behaviour.
 *
 * Links resolve to the built-out /home3 interior pages — those are the same
 * destinations, and five parallel copies of an inventory page would be five
 * things to keep in sync for no reviewable gain.
 */
      ...
      <section className="shell band-tight">
        <div className="grid gap-8 lg:grid-cols-12 lg:gap-12">
          <Reveal className="lg:col-span-7">
            ...
            <p className="t-display-l mt-5 text-ink text-balance">{company.positioning}</p>
          </Reveal>
            ...
            <dl className="mt-10 grid grid-cols-2 gap-x-6 gap-y-10">
              {keyFigures.map((figure) => (
                <div key={figure.label}>
                  ...
                  <p className="mt-3 text-sm font-medium leading-snug text-steel-600">
                    {figure.label}
                  </p>
                </div>
              ))}
        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-12">
            ...
            >
              ...
              <div className="relative transform transition-transform duration-[800ms] ease-[var(--ease-out-quart)] group-hover:-translate-y-2">
                <p className="eyebrow text-white/75">
                  ...
                  <span
                    aria-hidden
                    className="mx-3 inline-block h-px w-6 align-middle bg-white/40 transition-all duration-700 group-hover:w-10"
                  />
                  OWNED INVENTORY
                </p>
                <h3 className="t-heading mt-5 max-w-xl text-white md:text-3xl lg:text-4xl">
          ...
          <Reveal delay={80} className="lg:col-span-4">
            <div className="card flex h-full min-h-[400px] flex-col p-8 lg:min-h-[560px] lg:p-11">
              ...
              <h3 className="t-heading mt-5 text-ink md:text-3xl lg:text-4xl">
              ...
              <dl className="mt-9 flex-1 divide-y divide-steel-200">
                {headlineStock.map((row) => (
                  <div key={row.item} className="flex items-baseline justify-between gap-4 py-4">
                    <dt className="text-sm leading-relaxed text-steel-600 md:text-base">
                      {row.item}
                    </dt>
